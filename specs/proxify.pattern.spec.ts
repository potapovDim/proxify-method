import {expect as _expect} from 'assertior';
import {expect, AssertionError} from 'chai';
import {SomeControllerWithProxifyDataPattern} from './_setup.from.parent.proxify.pattern';

describe('Unit tests async parent method proxify pattern', function() {
  const someController = new SomeControllerWithProxifyDataPattern();
  ...
  it('[N] getArrayData', function() {
    try {
      someController.getArrayData().assertArray(1);
    } catch (error) {
      _expect(error.toString()).stringIncludesSubstring('.assertArray is not a function');
    }
  });
});
